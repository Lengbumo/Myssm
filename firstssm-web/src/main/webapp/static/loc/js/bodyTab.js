...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="'+my.window.ctxPath+'/myssm/manager/main"><i class="layui-icon" data-icon=""></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			var title = '';
			if(_this.find("i.seraph,i.layui-icon,i.fa").attr("data-icon") != undefined){
				if(_this.find("i.seraph").attr("data-icon") != undefined){
					title += '<i class="seraph '+_this.find("i.seraph").attr("data-icon")+'"></i>';
				} else if (_this.find("i.fa").attr("data-icon") != undefined) {
					title += '<i class="'+_this.find("i.fa").attr("data-icon")+'" data-icon="'+_this.find("i.fa").attr("data-icon")+'"></i>';
                } else{
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				//当前窗口内容
				var curmenu = {
					"icon" : _this.find("i.seraph").attr("data-icon")!=undefined ? _this.find("i.seraph").attr("data-icon") : _this.find("i.layui-icon,i.fa").attr("data-icon"),
	...
	$(".refresh").on("click",function(){  //此处添加禁止连续点击刷新一是为了降低服务器压力，另外一个就是为了防止超快点击造成chrome本身的一些js文件的报错(不过貌似这个问题还是存在，不过概率小了很多)
		if($(this).hasClass("refreshThis")){
			...
			layer.msg("您点击的速度超过了服务器的响应速度，请2秒后刷新！");
		}
	})
