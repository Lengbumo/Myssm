layui.use(['form', 'layedit', 'laydate', 'upload', 'jquery'], function () {	
    ...
    function initSelect() {
        if (!$.isNull($("#id").val())) {
        	// 获取传参
        	data =JSON.parse(decodeURIComponent($.getRequestParam().obj));
        	pbInitComCombox($,form,'menuSelect','parentId', data.parentId);
        } else {
        	pbInitComCombox($,form,'menuSelect','parentId');
        }
    }
    ...
    form.on('submit(btnSubmit)', function(data){
        ...
        $.ajax({
			url : my.window.ctxPath + '/myssm/manager/menu/saveData',
			data :formValues,
			type:'POST',
			dataType : 'json',
    ...
    form.on('select(moduleSelect)', function(data){
    	$("#moduleCode").val(data.elem[data.elem.selectedIndex].text);
    	form.render('select');
    });
