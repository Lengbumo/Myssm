layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'jquery'], function () {	
    ...
    //var layer = parent.layer === undefined ? layui.layer : top.layer; //弹层
    ...
    table.render({
        ...
        url: my.window.ctxPath + '/myssm/manager/menu/page',
        method: 'POST', //方式
        page: true,//是否开启分页
        ...
        cols: [[ //标题栏
            ...
            {type: 'checkbox'},
            {field: 'moduleCode', title: '所属模块编码', minWidth: 100, align: 'center', sort: true},
            {field: 'menuIcon', title: '菜单图标', minWidth: 100, align: 'center', sort: true, templet: iconTpl},
            {field: 'menuCode', title: '菜单编码', minWidth: 150, align: 'center', sort: true},
            {field: 'menuName', title: '菜单名称', minWidth: 100, align: 'center', sort: true},
            {field: 'menuSort', title: '排序', align: 'center', sort: true},
            {field: 'menuUrl', title: 'URL', minWidth: 450, align: 'center', sort: true},
            {field: 'createdBy', title: '创建人',  align: 'center', sort: true},
            {field: 'createdDate', title: '创建时间', minWidth: 175, align: 'center', sort: true},
    ...
    table.on('tool(tableList)', function (obj) { //注：tool是工具条事件名，table是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'btnDetail') {
            openDetail(data);
        }
        if (layEvent === 'btnUpdate') {
            openUpdate(data);
    ...
    function openDetail(data) {
    	// 获取详情
    	/*var url = my.window.ctxPath + '/myssm/manager/menu/view/' + data.id;
    ...
    function openUpdate(data) {
    	// 获取详情
    	/*var url = my.window.ctxPath + '/myssm/manager/menu/toEdit/' + data.id;
    	$.post(url, {}, function(resp) {
    		if (resp.rs) {
    			data = resp.data
    		}
        ...
        layer.open({
            ...
            success: function (layero) {
                ...
                btn.find('.layui-layer-btn0').on('click', function () {
                    ...
                    $.ajax({
                        url: my.window.ctxPath + '/myssm/manager/menu/bulkDelete',
                        data: ids,
