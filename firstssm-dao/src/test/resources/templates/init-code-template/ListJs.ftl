layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'jquery'], function () {	
    ...
    table.render({
        ...
        url: my.window.ctxPath + '/myssm/manager/${lowerBaseName?lower_case!''}/page',
        method: 'POST', //方式
        page: true,//是否开启分页
        ...
        cols: [[ //标题栏
			...
			<#list columns as data>
				<#if data.hiddenFlag != true>
            {field:'${data.name!''}', title:'${data.comment}', <#if data.dateFlag == true>minWidth:175,</#if><#if data.dateFlag != true><#if data.comment?length gt 5>minWidth:200,</#if></#if> align:'<#if data.bigdecimalFlag == true>right</#if><#if data.bigdecimalFlag != true>center</#if>', sort:true},
				</#if>
			</#list>
        ...
        layer.open({
            ...
            success: function (layero) {
                ...
                btn.find('.layui-layer-btn0').on('click', function () {
                    ...
                    $.ajax({
                        url: my.window.ctxPath + '/myssm/manager/${lowerBaseName?lower_case!''}/bulkDelete',
                        data: ids,
